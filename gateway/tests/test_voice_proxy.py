...
def capability(jti: str) -> str:
    payload = {
        ...
        "models": {"deepgram": [STT_MODEL], "elevenlabs": [TTS_MODEL]},
        "voices": {"elevenlabs": ["voice-test"]},
        "iat": 1_000,
        "exp": 1_300,
        "jti": jti,
        "max_requests": 20,
...
class MockProviders:
    ...
    async def elevenlabs(self, **request):
...
def test_metrics_exclude_high_cardinality_scope_and_audio():
    ...
    assert 'event="voice_stt",provider="deepgram",status="ok"' in metrics
    assert "org-a" not in metrics
    assert "audio" not in logs[0]
    assert "transcript" not in logs[0]


async def test_stt_buffers_utterance_and_calls_only_deepgram():
    async with gateway() as (base, providers, logs):
        async with websockets.connect(
            f"{base}/v1/stt/deepgram?model={STT_MODEL}",
        ...
        assert providers.requests[0][1]["model"] == STT_MODEL
        assert "texto sintético" not in "\n".join(logs)


async def test_tts_streams_elevenlabs_chunks_without_logging_text():
    async with gateway() as (base, providers, logs):
        async with websockets.connect(
            f"{base}/v1/tts/elevenlabs?model={TTS_MODEL}",
        ...
        assert providers.requests[0][1]["model"] == TTS_MODEL
