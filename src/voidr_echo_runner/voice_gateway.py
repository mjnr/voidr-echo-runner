"""Direct voice-provider clients backed by runtime-projected credentials."""
...
def assert_direct_provider_access_allowed() -> None:
    """Provider access is allowed only when both projected credentials exist."""
    if not os.environ.get("DEEPGRAM_API_KEY") or not os.environ.get("ELEVENLABS_API_KEY"):
        raise RuntimeError(
            "Direct voice providers require DEEPGRAM_API_KEY and ELEVENLABS_API_KEY "
            "from the runtime secret projection"
        )
...
class VoiceGatewayAudioEngine:
        ...
        async with asyncio.timeout(TTS_DEADLINE_S):
            async with self._http.stream(
                ...
                (
                    ...
                    f"{self.voice_id}?output_format=pcm_16000"
                ),
                headers={
                ...
                async for message in response.aiter_bytes():
                    ...
                    if chunks > MAX_AUDIO_CHUNKS or total > MAX_AUDIO_BYTES * 3:
                        raise RuntimeError("ElevenLabs TTS response exceeded limits")
                ...
                if pending or total == 0:
                    raise RuntimeError("ElevenLabs returned invalid PCM")
                if buffered:
    ...
    async def synthesize(self, text: str) -> bytes:
        ...
        if not chunks:
            raise RuntimeError("ElevenLabs returned no TTS audio")
        pcm = b"".join(chunks)
        valid, _, _ = validate_pcm_16k(pcm, self.sample_rate)
        if not valid:
            raise RuntimeError("ElevenLabs TTS returned invalid PCM16k audio")
        return pcm
    ...
    async def transcribe_stream(
        ...
        *,
        ...
        """Overlap capture and recognition through Deepgram's streaming API."""
        base = os.environ.get("DEEPGRAM_STREAMING_URL", "").strip() or DEEPGRAM_STREAMING_URL
        query = urlencode(
            {
                "model": "nova-2",
                "language": "pt-BR",
            ...
            async with websockets.connect(
                ...
                async def receive_results() -> None:
                    async for raw in ws:
                        ...
                        if not isinstance(sequence, (int, float)) or not isinstance(text, str):
                            raise RuntimeError("streaming STT returned an invalid result")
                        sequence_key = int(float(sequence) * 1000)
                        if payload.get("is_final"):
                            finals[sequence_key] = text.strip()
                        elif on_interim is not None:
                            on_interim(sequence_key, text)
    ...
    async def _transcribe_segment(self, pcm: bytes, index: int) -> str:
        ...
        if not isinstance(text, str):
            raise RuntimeError("Deepgram transcription returned no text")
        return text.strip()
