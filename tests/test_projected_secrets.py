...
def test_loads_direct_provider_secrets_into_runner(tmp_path: Path):
...
def test_managed_runtime_loads_projected_voice_credentials(tmp_path: Path):
    ...
    assert loaded == ["ELEVENLABS_API_KEY", "ECHO_MEDIA_GATEWAY_TOKEN"]
    assert env["ECHO_MEDIA_GATEWAY_TOKEN"] == "scoped-capability"
    assert "VOICE_GATEWAY_TOKEN" not in env
    assert env["DEEPGRAM_API_KEY"] == "inherited-key-must-be-removed"
    assert env["ELEVENLABS_API_KEY"] == "must-not-load"
